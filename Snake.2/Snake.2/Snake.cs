...
namespace Project___Snake
{
    public class Snake
    {
        ...
        //***********************************
        //Utökning för mig, säter en "timmer" på hur länge kontrollen skall vara igång.
        //***********************************
        public int SpecialMushroomTimer { get; set; }
        ...
        {
            //Om ormen har SpecialMushroom efekt på sig 
            if (SpecialMushroomTimer >= 0)
            {
     ...
     (direction == Direction.Right && newDirection == Direction.Left))
        ...
        //Utökning för mig
        //***********************************
        //Slumpa fram ormen rörsle 
        static Direction GetRandomDirection()
        {
            Random random = new Random();
            int RandomDirectionNumber = random.Next(0, 4);

            // Returnera motsvarande riktning på ormen baserande på det baserat på randomNumber
        ...
        public void Draw(Graphics graphics)
        {
            ...
            //Utökning för mig
            //***********************************
            bool BodyColor = true; // Flagga för att växla färg
            foreach (Rectangle segment in body)
            {
                ...
                using (Brush bodyColor = new SolidBrush(segmentColor))
                {
        ...
        public void EatFood(int score, int GrowBody)
        {
            ...
            //Utökning för mig
            //***********************************
            //Äter special frukten
            else if (GrowBody == 5)
            {
                Random random = new Random();
                int randomIndex = random.Next(0, allSnakes.Count);

                // Ta ut en orm från listan på slump sedan sätter den timmer på 250 (10 sekunder)
                Snake TheChosenOne = allSnakes[randomIndex];
                TheChosenOne.SpecialMushroomTimer=250;
